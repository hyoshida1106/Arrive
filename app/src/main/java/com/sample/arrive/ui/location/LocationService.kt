...
class LocationService: Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            stationInfoList.forEach { stationInfo ->
                if(stationInfo.valid && stationInfo.withinDistance(location)) {
                    if(stationInfo.stationId !in reportedIds) {
                        ...
                        val id = (stationInfo.stationId + 2).toInt()
                        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
                        notificationManager.notify(id, buildApproachingNotification(stationInfo.name))
        ...
        startForeground(1, buildStartNotification())
